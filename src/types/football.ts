...
export interface TacticalStyle {
   ...
   * 양쪽 윙어에 함께 적용되므로, 좌우 역할이 다른 팀은 아래 두 값으로 한쪽씩 덮어쓴다.
   */
  wingerTuck?: number;
  /** 왼쪽 윙어만 따로 지정한다. 생략하면 wingerTuck을 쓴다. */
  wingerTuckLeft?: number;
  /** 오른쪽 윙어만 따로 지정한다. 생략하면 wingerTuck을 쓴다. */
  wingerTuckRight?: number;
  /**
